...
public class TokenProvider(
    ...
    IdentityClaimsFactory identityClaimsFactory) : ITokenProvider
{
    ...
    public ClaimsPrincipal? GetPrincipalFromExpiredToken(string token)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning("Expired access token used an invalid security algorithm.");
                return null;
            }
        ...
        {
            _logger.LogWarning(exception, "Expired access token validation failed.");
            return null;
        }
    }
    ...
    private async Task<Result<TokenResponse>> CreateAsync(AppUserDto user, CancellationToken ct = default)
    {
        ...
        foreach (var claim in _identityClaimsFactory.Create(user.Roles, user.Claims))
        {
